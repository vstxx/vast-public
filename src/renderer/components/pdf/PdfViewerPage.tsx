...
import { getPdfViewerSource, hostnameFor } from '../../lib/url'
...
function pdfBlobPart(data: Uint8Array): ArrayBuffer {
...
function downloadBlob(data: Uint8Array, filename: string, mimeType: string): void {
  const blob = new Blob([pdfBlobPart(data)], { type: mimeType })
...
function PdfThumbnailItem({
  ...
  useEffect(() => {
    ...
    void pdfDocument
      ...
      .then((page) => {
        ...
        const scale = Math.min(1, 132 / baseViewport.width)
  ...
  return (
    <button
      ...
      type="button"
      onClick={onClick}
      className={`group w-full rounded-[26px] border p-3 text-left transition ${
        active
    ...
    >
      ...
      <div className="grid place-items-center rounded-[20px] bg-[#0d1015] p-3 ring-1 ring-white/[0.06]">
        ...
        {!rendered && <div className="h-[164px] w-[132px] rounded-[10px] bg-white/[0.05]" />}
      </div>
    </button>
  )
}
...
export function PdfViewerPage({ tab }: { tab: Tab }): JSX.Element {
  ...
  const mimeTypeRef = useRef<string>('application/pdf')
  ...
  const requestPdfPrint = async (): Promise<void> => {
    const bytes = documentBytesRef.current
    if (!bytes || !printAllowed || printing) return
    setPrinting(true)
    try {
      const result = await window.vast.pdf.print(bytes, fallbackPdfFilename(sourceUrl ?? tab.url, documentInfo.filename ?? tab.title))
  ...
  useEffect(() => {
    if (!sourceUrl || !containerRef.current || !viewerRef.current) return
    ...
    void (async () => {
      try {
        const result = await window.vast.pdf.load(stripHash(sourceUrl))
        if (!result.ok || !result.data) {
          throw new Error(result.error || 'Failed to fetch PDF bytes.')
        }
        if (disposed) return

        const data = result.data instanceof Uint8Array ? result.data : new Uint8Array(result.data)
        const filename = fallbackPdfFilename(sourceUrl, result.filename)
        documentBytesRef.current = data
        mimeTypeRef.current = result.mimeType || 'application/pdf'
    ...
    return () => {
      ...
      documentBytesRef.current = null
      void loadingTaskRef.current?.destroy().catch(() => undefined)
      loadingTaskRef.current = null
      pdfDocumentRef.current = null
    }
  }, [runtime, sourceUrl, tab.id, updateTab])
  ...
  }, [documentInfo.filename, pagesCount, printAllowed, printing, sourceUrl, tab.url])
  ...
  const handleDownload = (): void => {
    if (!documentBytesRef.current || !sourceUrl) return
    downloadBlob(documentBytesRef.current, fallbackPdfFilename(sourceUrl, documentInfo.filename), mimeTypeRef.current)
  }

  const handlePreviewDownload = (): void => {
    if (!documentBytesRef.current) return
    downloadBlob(documentBytesRef.current, fallbackPdfFilename(sourceUrl || tab.url, documentInfo.filename), mimeTypeRef.current)
  }
    ...
    return (
      <div className="grid h-full place-items-center bg-[radial-gradient(circle_at_top,rgba(255,162,94,0.12),transparent_26%),linear-gradient(180deg,#09090d,#050507)] p-6">
        <InternalEmptyState
          ...
          description="This internal viewer route does not contain a valid source URL for the PDF document."
        />
      </div>
    )
  }
      ...
      <section className="vast-glass-panel relative z-40 rounded-[22px] px-3 py-2">
        <div className="flex items-center gap-2">
          ...
          <div className="ml-1 flex shrink-0 items-center gap-1">
            ...
            <ToolbarButton title="Copy source link" onClick={handleCopyLink}>
              <Copy className="h-3.5 w-3.5" />
            </ToolbarButton>
            <ToolbarButton title="Open original URL" onClick={handleOpenOriginal}>
              <ExternalLink className="h-3.5 w-3.5" />
            </ToolbarButton>
            <ToolbarButton title="Download PDF" onClick={handleDownload}>
              <Download className="h-3.5 w-3.5" />
            </ToolbarButton>
            <ToolbarButton
              title={printAllowed ? 'Open print preview' : 'Printing disabled by document permissions'}
              onClick={() => void requestPdfPrint()}
              disabled={!printAllowed || printing}
              wide
            >
            ...
            <div className="min-h-0 flex-1 overflow-y-auto pr-1">
              {sidebarMode === 'thumbnails' && pdfDocumentRef.current && pagesCount > 0 ? (
                <div className="space-y-3">
                  {Array.from({ length: pagesCount }, (_item, index) => {
                    ...
                    return (
                      <PdfThumbnailItem
                        ...
                        label={pageLabels?.[index] ?? String(pageNumber)}
                  ...
                  <div className="absolute inset-0 z-10 grid place-items-center bg-[rgba(8,9,12,0.72)] backdrop-blur-sm">
                    <div className="text-center">
                      ...
                      <div className="mt-2 max-w-md text-sm leading-6 text-vast-soft">
                        Vast is fetching the document through the internal pipeline and preparing page rendering, search indexes, and navigation targets.
                      </div>
                    </div>
                  </div>
                )}
                    ...
                    <InternalEmptyState
                      ...
                      action={
                        <div className="flex flex-wrap justify-center gap-2">
                          <button type="button" onClick={handleOpenOriginal} className="vault-action-button">
                            <ExternalLink className="h-4 w-4" />Open original URL
                          </button>
                          <button type="button" onClick={() => runtime.openUrlInNewTab(tab.url)} className="vault-action-button bg-vast-cyan text-black">
