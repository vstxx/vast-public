...
export function dashboardPage(extensions: DashboardExtension[], session: HubSession, categories: string[], termsAccepted: boolean): string {
  ...
  const listings = extensions.map((extension) => {
    ...
    return `<article class="panel listing-card">
      ...
      <div class="upload-section"><div class="upload-section__heading"><h3>Release assets</h3><p>Add only what is ready for validation or review.</p></div><div class="upload-grid">
        <form class="upload-form" data-extension-id="${escapeHtml(extension.id)}"><div class="upload-form__copy"><span class="upload-form__number">01</span><div><strong>Extension package</strong><span>Validated .vext release bundle</span></div></div><label class="file-picker"><input required name="package" type="file" accept=".vext,application/vnd.vast.extension+zip"><span class="file-picker__surface"><span class="file-picker__action">Choose package</span><span class="file-picker__name" data-file-name data-empty="No file selected">No file selected</span></span></label><button class="button button--quiet" type="submit">Validate upload</button></form>
        <form class="upload-form media-upload-form" data-extension-id="${escapeHtml(extension.id)}" data-kind="icon"><div class="upload-form__copy"><span class="upload-form__number">02</span><div><strong>Catalog icon</strong><span>Square PNG, JPEG or WebP</span></div></div><label class="file-picker"><input required type="file" accept="image/png,image/jpeg,image/webp"><span class="file-picker__surface"><span class="file-picker__action">Choose image</span><span class="file-picker__name" data-file-name data-empty="No file selected">No file selected</span></span></label><button class="button button--quiet" type="submit">Upload icon</button></form>
        <form class="upload-form media-upload-form" data-extension-id="${escapeHtml(extension.id)}" data-kind="screenshot"><div class="upload-form__copy"><span class="upload-form__number">03</span><div><strong>Catalog screenshot</strong><span>Up to five PNG, JPEG or WebP previews</span></div></div><label class="file-picker"><input required type="file" accept="image/png,image/jpeg,image/webp"><span class="file-picker__surface"><span class="file-picker__action">Choose image</span><span class="file-picker__name" data-file-name data-empty="No file selected">No file selected</span></span></label><button class="button button--quiet" type="submit">Add screenshot</button></form>
      </div></div>
...
export function reviewPage(items: ReviewItem[], session: HubSession): string {
  const queue = items.map((item) => {
    ...
    const adminSelfReview = ownedByReviewer && session.publisher.role === 'admin'
    const separateReviewerRequired = ownedByReviewer && !adminSelfReview
    return `<article class="panel review-card"><div class="review-card__header"><div><div class="review-card__badges"><span class="badge">Pending review</span>${adminSelfReview ? '<span class="badge badge--admin">Admin self-review</span>' : separateReviewerRequired ? '<span class="badge">Separate reviewer required</span>' : ''}</div><h2>${escapeHtml(item.extensionName)} <span>${escapeHtml(item.version)}</span></h2><p class="meta">${escapeHtml(item.publisherName)} &middot; submitted ${escapeHtml(item.submittedAt)}</p></div><span class="review-card__id">${escapeHtml(item.submissionId)}</span></div>
