...
export function reviewPage(items: ReviewItem[], session: HubSession): string {
  const queue = items.map((item) => {
    const permissions = [...item.permissions.chrome, ...item.permissions.hosts, ...item.permissions.vast]
    const adminSelfReview = session.publisher.role === 'admin' && item.publisherId === session.publisher.id
    return `<article class="panel review-card"><div class="review-card__header"><div><div class="review-card__badges"><span class="badge">Pending review</span>${adminSelfReview ? '<span class="badge badge--admin">Admin self-review</span>' : ''}</div><h2>${escapeHtml(item.extensionName)} <span>${escapeHtml(item.version)}</span></h2><p class="meta">${escapeHtml(item.publisherName)} &middot; submitted ${escapeHtml(item.submittedAt)}</p></div><span class="review-card__id">${escapeHtml(item.submissionId)}</span></div>
      ...
      ${adminSelfReview ? '<p class="admin-review-note">Trusted administrator override: this release belongs to your publisher account. Approval is recorded as an administrator self-approval in the audit log.</p>' : ''}<div class="review-actions"><button class="button primary" data-action="approve" data-review="${escapeHtml(item.submissionId)}" data-url="/v1/review/submissions/${escapeHtml(item.submissionId)}">${adminSelfReview ? 'Approve and publish' : 'Approve and sign'}</button><button class="button button--quiet" data-action="changes" data-review="${escapeHtml(item.submissionId)}" data-url="/v1/review/submissions/${escapeHtml(item.submissionId)}">Request changes</button><button class="button danger" data-action="reject" data-review="${escapeHtml(item.submissionId)}" data-url="/v1/review/submissions/${escapeHtml(item.submissionId)}">Reject</button></div>
    </article>`
  }).join('')
