...
test('full Vast backup creates a .vastbackup zip with manifest, checksums, and expected data', async () => {
  ...
  try {
    ...
    await mkdir(join(dataRoot, 'Partitions', 'vast-default', 'Service Worker', 'CacheStorage'), { recursive: true })
    await mkdir(join(dataRoot, 'Partitions', 'vast-default', 'Service Worker', 'ScriptCache'), { recursive: true })
    await mkdir(join(dataRoot, 'Partitions', 'vast-default', 'blob_storage'), { recursive: true })
    ...
    await writeFile(join(dataRoot, 'Partitions', 'vast-default', 'Service Worker', 'CacheStorage', 'cache-entry'), 'recoverable cache', 'utf8')
    await writeFile(join(dataRoot, 'Partitions', 'vast-default', 'Service Worker', 'ScriptCache', 'script-entry'), 'recoverable script cache', 'utf8')
    await writeFile(join(dataRoot, 'Partitions', 'vast-default', 'blob_storage', 'blob-entry'), 'recoverable blob cache', 'utf8')
    ...
    assert.equal(report.skippedFiles.includes('Partitions/vast-default/Service Worker/CacheStorage/cache-entry'), true)
    assert.equal(report.skippedFiles.includes('Partitions/vast-default/Service Worker/ScriptCache/script-entry'), true)
    assert.equal(report.skippedFiles.includes('Partitions/vast-default/blob_storage/blob-entry'), true)
    ...
    assert.equal(entries.some((entry) => entry.includes('/CacheStorage/')), false)
    assert.equal(entries.some((entry) => entry.includes('/ScriptCache/')), false)
    assert.equal(entries.some((entry) => entry.includes('/blob_storage/')), false)
